...
mode<-optimize(f1,interval=c(0,10))$minimum
M_best<-f(mode)
...
result<-tibble(M=seq(M_best,5,by=0.025))%>%
  mutate(Y=map(M,function(x) rejection_sampling(x,f=f,g=g)),
          acceptance_rate=lengths(Y)/1000)
...
## Do not modify this line!
acceptance_plot<-ggplot(data=result,aes(x=M,y=acceptance_rate))+
  geom_line()+
  labs(title = "The acceptance rate decreases with M",
       x="M",
       y="Acceptance rate")+
  theme_light()
...
mode2<-optimize(g22,c(-10,10))$minimum
M2<-f(mode2)/g2(mode2)

set.seed(1)
seed<-.Random.seed
result2<-tibble(y=rnorm(n,mean=mode,sd=3),
                u=runif(n),
...
good_rejection_sampling_plot<-ggplot(data=result2,aes(x=y))+
  geom_point(aes(y=M2*u*g2(y),color=selected))+
  stat_function(fun = f,size=2)+
  stat_function(fun=function(x) M2*g2(x),color="red",size=2)+
  labs(title = "Acceptance illustration",
       x="x",
       y="Density")+
  theme_light()
