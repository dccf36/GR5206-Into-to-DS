...
next_in_seq<-function(coeffs,previous){
  ...
  tmp<-map2(coeff_x_previous,1:length(previous),aux)
  tmp<-reduce(tmp,xor)
  tmp<-xor(tmp,as.binary(previous[length(previous)]))
...
sobol<-function(coeffs,n,init){
  ...
  output<-c(output,next_in_seq(coeffs,previous))
  previous<-c(next_in_seq(coeffs,previous),previous)[1:length(init)]
  }
  return(post_process(output))
}
...
simul_tibble<-tibble(sequence=1:50,
  ...
  pivot_longer(cols=c(2:4),names_to = "type",values_to = "value")
...
#
## Do not modify this line!
simul_plot<-ggplot(data=simul_tibble,aes(x=sequence,y=value))+
  geom_point()+
  facet_wrap(~type)+
