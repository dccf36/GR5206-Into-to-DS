...
boot_ratio<-function(nboot,df){
  tf=tibble(b=1:nboot,
         df_b=map(b,~boot_df(df)),
         theta_hat_b=map(df_b,ratio))
  out=(pull(tf,theta_hat_b))
  return(unlist(out))
...
results<-tibble(nboot=c(50,100,200,500,1000),
       ...
       bias=map_dbl(theta_hat_b,mean)-theta_hat,
...
rmean(c(7.06, 4.06),c(1.48, 6.18))
rratio(p = c(0.5, 0.5),tibble(y = c(7.06, 4.06), z = c(1.48, 6.18)))
...
boot_p_ratio<-function(df){
  n<-nrow(df)
  p<-tabulate(sample(n,size=n,replace = T),nbins=n)/n
  theta<-rratio(p,df)
  tibble(theta=theta,p=list(p))
}
boot_p_ratio(tibble(y = c(7.06, 4.06), z = c(1.48, 6.18)))

mean_p<-function(p){
  reduce(p,`+`)/length(p)
}
mean_p(list(c(0.5, 0.2), c(0.1, 0.3), c(1, 2)))
...
set.seed(1)
df1=tibble(y = c(7.06, 4.06), z = c(1.48, 6.18))
nboot=3
boot_ratio2(nboot, df1)
...
set.seed(1)
seed2<-.Random.seed
results2 <- tibble(nboot = c(50, 100, 200, 500, 1000), theta_hat_p_hat = map(nboot,~boot_ratio2(.,df))) %>%
  unnest(theta_hat_p_hat) %>% mutate(theta_hat = map_dbl(p_hat_b,~rratio(.,df))) %>%
  mutate(bias = map_dbl(theta_hat_b, mean) - theta_hat) %>%
  mutate(theta_hat_corrected= theta_hat-bias) %>%
  mutate(type="improved") %>% select(-theta_hat_b, -p_hat_b, -theta_hat)
results2
