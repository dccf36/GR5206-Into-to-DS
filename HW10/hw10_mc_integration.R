...
ci_unif<-df%>%
  summarize(integral=mean(f_3),
            se=sqrt(var(f_3)/n))%>%
  mutate(lower=integral-1.96*se,
...
ci_anti<-df2%>%
  ...
  mutate(lower=integral-se*1.96,
         upper=integral+se*1.96)
...
cum_stats<-df3%>%
  mutate(cummean_f_3=cummean(f_3),
         ...
         cumcov_f_2_x_f_3=cummean(f_2*f_3)-cummean_f_2*cummean_f_3,
         cum_c=-cumcov_f_2_x_f_3/cumvar_f_2)%>%
  filter(! is.na(cum_c))
cum_stats %>% select(-c(s, f_3, f_3_anti, f_2))
...
errors<-cum_stats%>%
  ...
  pivot_longer(cols=unif:control,names_to="sample_type",values_to = "value")
errors
...
## Do not modify this line!
library(ggplot2)
error_plot<-ggplot(data=errors,aes(x=sequence,y=value,colour=sample_type))+
  geom_line()+
  coord_cartesian(xlim = c(0, 1500))+
  labs(title = "Using antithetic samples and control variates accelerates convergence and reduces variance",
       y="MC error",
       colour="Sample type")+
  theme_light()
error_plot
