...
norm_plot<-ggplot(data)+
  ...
  geom_line(data=filter(data,y==1),aes(x = x, y = z),color="red")+
...
seed3<-.Random.seed
original<-rnorm(n)
h<-function(x,f) g(x)/f
fit<-tribble(~x,~name,~f,
             uniform,'uniform',1,
             original,'original',dnorm(original,0,1))%>%
  mutate(g_over_f=map2(x,f,h))
fit
...
subsamples<-tribble(~width,~samples,
  ...
  mutate(g_over_f=map2(width,samples,function(width, samples) g(samples) / dunif(samples, 2 - width/2, 2 + width/2)))
...
## Do not modify this line!
variance_plot<-ggplot(data=result2,aes(y = mean, x = width))+
  geom_line()+
