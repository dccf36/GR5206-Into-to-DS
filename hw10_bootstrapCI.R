...
boot_parametric_factory<-function(df,model){
  function(){
    pred=fitted(model(df))
    resid=resid(model(df))
    resid_re=sample(resid,length(resid),replace = T)
    df$mpg<-pred+resid_re
    return(df)
  }
}
set.seed(0)
...
result_boot_tidy<-result_boot%>%
  ...
  unnest(value)%>%
  mutate(full=ifelse(term=='(Intercept)',coef(fit_mtcars)[1],coef(fit_mtcars)[2]))
result_boot_tidy
...
#
## Do not modify this line!
ci_basic<-function(thetab,thetahat,alpha=0.95){
  qt=quantile(thetahat,c((1-alpha)/2,(1+alpha)/2))
  tibble(lower=2*thetab-qt[2],
         upper=2*thetab-qt[1])
}

ci_quantile<-function(thetab,thetahat,alpha=0.95){
  qt=quantile(thetahat,c((1-alpha)/2,(1+alpha)/2))
  tibble(lower=qt[1],
         upper=qt[2])
}
...
ci_bootstrap<-result_boot_tidy%>%
  group_by(boot,term)%>%
  summarize(basic=map(full[1],~ci_basic(.x,estimate)),
            percentile=map(full[1],~ci_quantile(.x,estimate)))%>%
