...
## Do not modify this line!
density_plot<-ggplot(data=X,aes(x1,x2))+
...
f<-function(x){
  ...
  return(beta*x[1]*x[2]*(1-x[1]-x[2]))
}
f(c(0.3, 0.3))

f1<- function(x) -f(x)

M<-abs(optim(c(0.5,0.5),f1,lower=c(0,0),upper=c(1,1),method = "L-BFGS-B")$value)
...
#    - `theme_light()` to set the theme to light.
## Do not modify this line!
sample_plot<-ggplot(data=samples,aes(x1,x2))+
