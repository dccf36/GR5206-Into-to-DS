...
est1_cauc<-c(mean(g_over_f1_cauc(x1_cauc)),sqrt(var(g_over_f1_cauc(x1_cauc))/n))
...
est2_cauc<-c(0.5 - mean(g_over_f2_cauc(x2_cauc)),
             sqrt(var(g_over_f2_cauc(x2_cauc))/n))
...
#         [1] 0.7469 0.0064
## Do not modify this line!
p2<-(pnorm(1,0,1/sqrt(2))-pnorm(0,0,1/sqrt(2)))*sqrt(pi)
n<-1e3
set.seed(0)
seed<-.Random.seed
x_exp<-runif(n)
...
est1_exp<-c(mean(g_over_f_exp(x_exp)),
            sqrt(var(g_over_f_exp(x_exp))/n))
...
## Do not modify this line!
est2_exp<-c(mean((g_over_f_exp(1-x_exp)+g_over_f_exp(x_exp))/2),
            sqrt(var((g_over_f_exp(1-x_exp)+g_over_f_exp(x_exp))/2)/n))
cor1_exp<-cor(g_over_f_exp(1-x_exp),g_over_f_exp(x_exp))
round(est2_exp, 4)
...
est3_exp<-c(mean(g_over_f_exp(x_exp)+c*(h(x_exp)+1/3)),
            sqrt(var(g_over_f_exp(x_exp)+c*(h(x_exp)+1/3))/n))
round(est3_exp, 4)
...
est_circ<-c(mean(g_over_f_circ(x_circ,y_circ)),
