...
speed<-c(28, -44, 29, 30, 26, 27, 22, 23, 33, 16, 
         24, 29, 24, 40, 21, 31, 34, -2, 25, 19)
speed_hist<-ggplot(tibble(speed=speed),aes(speed))+
...
#    - `theme_light()` to set the theme to light.
## Do not modify this line!
bstrap_hist<-ggplot(bstrap,aes(muhat_null))+
  geom_histogram(bins=30,color="white")+
...
boot_pval<-function(muhat,mu0,muhat_null){
  length(muhat_null[abs(muhat_null-mu0)>=abs(muhat-mu0)])/length(muhat_null)
}
boot_pval(30, 31, speed)
p_value<-boot_pval(mean(speed),mu0,pull(bstrap,muhat_null))
# 6. (1)
...
boot_speed_null2<-boot_null(newspeed,mu0)
...
p_value2<-boot_pval(mean(newspeed),mu0,pull(bstrap2,muhat_null))
