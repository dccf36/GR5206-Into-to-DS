...
set.seed(0)
seed<-.Random.seed
df<-tibble(u=runif(1000),x=Finv(u))
...
g1<-df%>%
  ...
  stat_function(fun = dcauchy,pars=list(loction=0,scale=1),color="red")+
...
## Do not modify this line!
g2<-ggplot(data=df,aes(sample=x))+
  geom_qq(distribution = qcauchy)+
  stat_qq_line(color="red",distribution = qcauchy)+
