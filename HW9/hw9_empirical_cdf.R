...
spotify<-spotify%>%
  mutate(popularity_label=fct_recode(cut(popularity,breaks =c(0, 30, 70, 100)),
                                     "unpopular"="(0,30]",
                                     "fair"="(30,70]",
                                     "popular"="(70,100]"))%>%
  drop_na()
...
## Do not modify this line!
library(ggplot2)
danceability_dist<-ggplot(data=spotify,aes(x=danceability, y = ..density..))+
  geom_histogram(binwidth = 0.04,color="black",fill="orange")+
...
nll_gamma_factory <- function(x){
  ...
  function(par){
    a<-par[1]
    s<-par[2]
    log_lik=-(a-1) *slx +n*lgamma(a) + n*a*log(s)+sx/s
    return(log_lik)
  }
}
...
liveness_gamma_plot<-ggplot(data=spotify_pop)+
  geom_histogram(aes(x = liveness, y = ..density..),binwidth = 0.02,color="black",fill="orange")+
