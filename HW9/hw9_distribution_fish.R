...
nll_pois_factory<-function(x){
  ...
  function(par){
    lambda=par[1]
    pi=par[2]
    L<-y*log(pi+(1-pi)*exp(-lambda))+(n-y)*log(1-pi)-(n-y)*lambda+n*x_bar*log(lambda)
    return(-L)
  }
}
...
par_pois2<-function(x){ 
  par0<-par_pois(x)
  nll_piso_x<-nll_pois_factory(x)
  fit<-optim(par0,nll_piso_x,method="L-BFGS-B",hessian=T,lower=c(0.001,0),upper = c(Inf,0.999))
  fisher_info<-solve(fit$hessian)
  se<-sqrt(diag(fisher_info))
  tibble(parameter=c("lambda","pi"),value=fit$par,se=se,lower=fit$par-1.96*se,upper=fit$par+1.96*se)
}
...
dparams_mle<-list(mu=cnt_pois_mle[1],
                  sigma=cnt_pois_mle[2])

g3<-ggplot(data=data,aes(sample=cnt))+
