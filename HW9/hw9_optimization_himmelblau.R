...
library(lattice)
library(tidyverse)
xy<-crossing(x=seq(-4,4,length.out = 101),
           y=seq(-4,4,length.out = 101))%>%
...
#    4 x 4 square.
## Do not modify this line!
himmelblau_plot<-wireframe(fnxy~x*y,data=xy,shade=TRUE,
...
contour<-ggplot(data=xy,aes(x = x, y = y, z = fnxy))+
  ...
  labs(title="Contour of himmelblau function",
       x="X",
       y="Y",
       fill="Z")+
  theme_light()
...
softline_dir<-crossing(x=c(1,-1),y=c(1,-1))%>%
  ...
  mutate(a=map_dbl(d0,function(x) softline(x0,d0=x,f=himmelblau)))%>%
  mutate(optimum=map2(d0,a,function(x,y) x0+x*y))%>%
  mutate(method=paste0("softline: a=",round(a,2)))
...
pts<-tribble(~optimum,~method,
             ...
             optim_minima,"optim")%>%
  bind_rows(dplyr::select(softline_dir,optimum,method))%>%
