...
stocks_data_longer<-stocks_data%>%
  pivot_longer(cols=2:11,names_to="stock",values_to="return")
...
dvec<-rep(0,10)
row1<-rep(1,10)
row2<-map_dbl(stocks_data[,2:11],mean)
row3<-diag(10)
Amat<-rbind(row1,row2,row3,-row3)
...
#    # … with 8 more rows
## Do not modify this line!
portfolio<-tibble(stock=colnames(stocks_data)[2:11])%>%
  mutate(proportion=round(sol[[1]],3))%>%
...
## Do not modify this line!
portfolio_plot<-ggplot(data=portfolio,aes(y=proportion,x=stock))+
