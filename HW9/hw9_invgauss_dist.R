...
obj_invgauss<-function(par,x){
  ...
  moments_est=mom_invgauss(par)
  dif=(moments-moments_est)^2
  return(sum(dif))
}
...
nll_invgauss<-function(par,x){
  return(-sum(dinvgauss(x,mean=par[1],shape=par[2],log=TRUE)))
}
set.seed(100)
seed2<-.Random.seed
...
## Do not modify this line!
par2
mu_hat<-sum(random_data)/length(random_data)
mu_hat
shape_hat<-1/(1/1000*sum(1/random_data-1/mu_hat))
shape_hat
...
## Do not modify this line!
ks.test(random_data,pinvgauss,mean=par2[1],shape=par2[2])
...
g2<-ggplot(data=random_data_tbl,aes(x=random_data))+
  ...
  stat_function(fun= partial(dinvgauss, mean = par1[1], shape = par1[2]),aes(col="MOM"))+
  stat_function(fun=partial(dinvgauss,mean=par2[1],shape=par2[2]),aes(col="MLE"))+
  stat_function(fun=partial(dinvgauss,mean=2,shape=3),aes(col="True Value"))+
