...
multi_step<-function(f,x,h,n_iterations){
  m<-accumulate(rep(h,n_iterations),one_step,f=f,.init=x)
  out<-enframe(m)
  colnames(out)<-c('step','x')
  return(out)
}
multi_step(f, 1, h, 2)
...
newton_steps_tibble<-tibble(start=seq(-7,7,2))%>%
  mutate(steps=map(start, function(x) multi_step(f,x,h,2)))%>%
  unnest(steps)%>%
...
#    - add `theme_light()`
## Do not modify this line!
newton_steps_plot<-ggplot(data=newton_steps_tibble,aes(x,f))+
  stat_function(fun=f)+
...
plot_with_root<-f_plot+
  geom_point(aes(x=root,y=f(root)),col="red")+
