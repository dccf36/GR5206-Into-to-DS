...
obj_lnorm_factory<-function(x){
  ...
  var_hat<-var(x)
  function(par){
    return(sum((mom_lnorm(par)-c(mu_hat,var_hat))^2))
  }
}
...
par_lnorm<-function(x){
  fit<-optim(c(1,1),obj_lnorm_factory(x),hessian = TRUE)
  par<-fit$par
  value<-fit$value
